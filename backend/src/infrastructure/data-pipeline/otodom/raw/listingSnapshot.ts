...
/**
 * Otodom “page not found” / removed listing — no LD, no attributes, no core facts, and tell-tale main text.
 * Skip during sanitize so these rows never reach tagging, DB import, or the API as real offers.
 */
export function isUnavailableOtodomListingSnapshot(item: OtodomListingSnapshotItem): boolean {
    const text = item.mainTextRaw?.toLowerCase() ?? ''
    const hasNoStructuredData =
        !item.ldJsonRaw &&
        item.attributesRawJson === '{}' &&
        item.priceAmount == null &&
        item.areaSqm == null &&
        item.rooms == null
    const hasUnavailableText =
        text.includes('nie znaleźliśmy strony') || text.includes('strona mogła zostać usunięta')
    const hasMissingTitle = !item.title || item.title === 'Untitled'
    return hasNoStructuredData && hasMissingTitle && hasUnavailableText
}
